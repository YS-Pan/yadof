...
| `automatic toDo obsolete rule` | One of two policies checked whenever an automatic toDo is read. The default automatic policy archives it when either its time limit is exceeded (seven days by default) OR project changes invalidate its content. The `manual` policy disables both automatic conditions. Completion archiving remains separate. |
