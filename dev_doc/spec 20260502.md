...
| workflow | 将优化变量转化为 rawData 的流程。 | workflow 可以调用仿真软件，也可以调用自定义 Python 代码。具体仿真文件、外部软件、rawData 名称和目标含义都属于当前任务定义；workflow 只输出 rawData，不输出 cost。 |
| com 文件 | 连接 workflow 与具体仿真软件或模拟程序的适配层。 | 当某个任务实际需要某个 com 文件时，该文件应放在 `project/job_template/`，随 workflow 一起复制进 job 文件夹。 |
| com_lib | 非核心适配器存放区。 | 可保存暂未启用或常用适配器的参考/源文件，但 workflow 不应直接 import `com_lib`；用户启用某个 com 时应手动复制到 `job_template/`。 |
| test_com.py | 纯 Python synthetic 评估适配器。 | 已移动到 `project/com_lib/`，作为快速模拟或未来测试参考；只有复制到 `job_template/` 后才会被具体任务使用。 |
...
8. 当某个任务使用 HFSS 时，用户可将 `hfss_com.py` 和任务自己的 `.aedt` 文件放在 `project/job_template/` 中；其他任务可替换为别的适配器或输入文件。
9. `project/com_lib/hfss_com.py` 保留 HFSS 适配器的参考/源副本，`project/com_lib/test_com.py` 保留为 synthetic 适配器参考；active workflow 只从 `job_template/` 同目录导入 active com 文件。
10. 仿真模型文件是任务输入，不是框架固定文件；每个任务可以替换为自己的 `.aedt` 文件、其他仿真文件或纯 Python 输入。
...
存放 workflow 可调用的仿真或模拟适配器的参考/源副本。

约束：

1. `com_lib/` 不是核心模块，不提供核心模块之间的通信 API。
2. `com_lib/` 中的文件不作为运行时依赖；active `workflow.py` 只调用已经复制到 `job_template/` 同目录的适配器文件。
3. `com_lib/hfss_com.py` 是 HFSS 适配器的参考/源副本，`com_lib/test_com.py` 保留为 synthetic 适配器参考或未来轻量测试入口。
4. 当用户需要某个 `com_lib` 文件参与任务时，应手动将它复制到 `job_template/`。
5. `optimize`、`surrogate`、`evaluate_manager`、`recorded_data` 不应直接依赖某个具体 `com_lib` 适配器。
...
4. 默认测试应避免无意启动真实 HFSS；需要真实 AEDT 的 smoke test 应通过显式环境变量或手动命令开启。
...
1. 从 `job_template` 复制 workflow、参数文件、仿真输入文件，以及已经放入模板目录的任务 com 文件。
2. 写入 `jobs/` 下对应 job 文件夹。
3. 如果 active workflow 使用 `job_template/hfss_com.py`，该适配器会随 job 一起复制，但仍不复制 `calc_cost.py`。
4. `com_lib/hfss_com.py` 保存 HFSS 适配器的参考/源副本；`test_com.py` 只作为 `com_lib` 中的 synthetic 参考适配器保留。
...
4. 任务可选使用的适配器文件，例如 HFSS 或 synthetic 适配器。
...
2. `workflow.py`
3. `calc_cost.py`
4. `job_template` 中被 workflow 调用的适配器文件，例如 `hfss_com.py` 或 `test_com.py`
5. 真实仿真模型文件或自定义输入文件，例如用户自己的 `.aedt`、数据文件或脚本
...
4. 任务可以按需接入 `hfss_com.py` 等适配器，并保持 workflow/calc_cost 分离。
5. 后续可以继续把 `maxwell_com.py`、`twinbuilder_com.py` 等适配器放入 `com_lib` 作为库；启用时复制到 `job_template/`，而不是修改核心优化框架。
...
1. 新项目只是架构上支持这些可能性。
2. 当前只接入 HFSS 适配器和保留 synthetic 适配器，不需要一次性写完所有 `_com.py` 文件。
3. 新仿真软件可先在 `project/com_lib/` 保存适配器；启用某个任务时，把需要调用的适配器复制到 `project/job_template/`，由当前任务的 `workflow.py` 同目录导入。
4. 默认 workflow 可以是真实仿真，也可以在新任务中替换为纯 Python synthetic workflow。
