...
| workflow | 将优化变量转化为 rawData 的流程。 | workflow 可以调用仿真软件，也可以调用自定义 Python 代码。当前默认任务使用 HFSS/PyAEDT 运行 `Metal_recon_ant.aedt`，但仍只输出 rawData，不输出 cost。 |
| com 文件 | 连接 workflow 与具体仿真软件或模拟程序的适配层。 | 当某个任务实际需要某个 com 文件时，该文件应放在 `project/job_template/`，随 workflow 一起复制进 job 文件夹。 |
| com_lib | 非核心适配器存放区。 | 可保存暂未启用或当前启用适配器的参考/源文件，但 workflow 不应直接 import `com_lib`；用户启用某个 com 时应手动复制到 `job_template/`。 |
| test_com.py | 纯 Python synthetic 评估适配器。 | 已移动到 `project/com_lib/`，作为快速模拟或未来测试参考；当前默认 workflow 不再使用它。 |
...
8. 当前默认任务使用 `project/job_template/hfss_com.py` 和 `Metal_recon_ant.aedt` 做真实 HFSS 仿真。
9. `project/com_lib/hfss_com.py` 保留当前 HFSS 适配器的参考/源副本，`project/com_lib/test_com.py` 保留为 synthetic 适配器参考；当前 workflow 只从 `job_template/` 同目录导入 active com 文件。
10. `Metal_recon_ant.aedt` 是当前昂贵评估用仿真模型；其他任务可以替换为自己的仿真文件。
...
2. `com_lib/` 中的文件不作为运行时依赖；当前 `workflow.py` 调用的是同目录 `job_template/hfss_com.py`。
3. `com_lib/hfss_com.py` 是当前 HFSS 适配器的参考/源副本，`com_lib/test_com.py` 保留为 synthetic 适配器参考或未来轻量测试入口。
...
3. 当前 HFSS workflow 会复制 `job_template/hfss_com.py`，但仍不复制 `calc_cost.py`。
4. `com_lib/hfss_com.py` 保存当前 HFSS 适配器的参考/源副本；`test_com.py` 只作为 `com_lib` 中的 synthetic 参考适配器保留。
...
4. 当前任务使用的 HFSS 适配器和保留的 synthetic 适配器。
...
5. 真实仿真模型文件，例如 `Metal_recon_ant.aedt`
...
4. 当前默认任务已接入 `hfss_com.py`，并保持 workflow/calc_cost 分离。
