...
- Runnable jobs use whichever task files and active adapter files are placed in `project/job_template/`; `project/com_lib/hfss_com.py` is the synchronized reusable HFSS reference copy, and `project/com_lib/test_com.py` remains available only as a synthetic adapter reference.
- The current surrogate is a conditional INR rawData deep ensemble adapted from the shorten-style direction while preserving the rawData-first public API.
- The surrogate keeps the small `api.py` surface from the earlier implementation, but `runtime.py` now delegates neural model construction/training/prediction to `surrogate/modeling.py`.
