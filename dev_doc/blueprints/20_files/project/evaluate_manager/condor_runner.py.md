...
- Write Windows HTCondor submit files from values exposed by `project.config.all` through `evaluate_manager.config` and the adaptive `resource_requests` helper.
- Invoke an optional `after_jobs_submitted` callback after successful submissions and before polling outputs.
- Poll terminal job state or complete returned outputs, collect rawData and metadata, query final `condor_history`/held-job ClassAds for resource measurements, and turn failures/timeouts into `JobResult` rows.
...
- Submit files use `executable = workflow.py`, omit the workflow argument line, set `transfer_executable = True`, and use the calculated `request_cpus`, `request_memory`, and `request_disk` values. They also emit bounded `retry_request_memory` and `retry_request_disk` doubling ladders.
...
- `condor_resource_usage()` reads a final JSON ClassAd from `condor_history`, then
  ...
  execute-directory `DiskUsage`, and related request/CPU values as diagnostics but
