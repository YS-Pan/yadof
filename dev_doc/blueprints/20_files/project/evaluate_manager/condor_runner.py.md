...
- Write Windows HTCondor submit files from values exposed by `project.config.all` through `evaluate_manager.config`.
...
- The generic environment comes from `project.config.all`; active software-specific entries are contributed through `project.config.specific`. For the current HFSS extension, keep its solver core count aligned with `HTCONDOR_REQUEST_CPUS`.
