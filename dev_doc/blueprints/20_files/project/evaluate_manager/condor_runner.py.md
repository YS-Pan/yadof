...
- Submit files use `executable = workflow.py`, `transfer_executable = True`, and `request_cpus`, `request_memory`, and `request_disk` from config in the default direct-workflow mode. Interpreter mode can still use `executable = <configured python>`, `arguments = workflow.py`, and `transfer_executable = False` when explicitly selected.
...
- Direct `workflow.py` submission matches the Windows HTCondor debug reference and avoids the fragile absolute-interpreter executable pattern.
