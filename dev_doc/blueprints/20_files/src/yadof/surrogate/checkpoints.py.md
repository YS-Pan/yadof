...
## Functionalities
- Compute a semantic state signature over format/method/policy, parameters,
...
- Immutable publication artifacts and committed unique manifests live below
  `runs/<semantic-state>/components/surrogate/`.
- Manifests explicitly declare `format_version`, `surrogate_method`,
  `training_policy`, semantic signature, and run/component namespaces.
