...
- `blueprints/10_modules/` and `blueprints/20_files/` contain module/file blueprints that can guide recreation of equivalent behavior and should hold module-specific historical reference ancestry when needed.
...
- Default context gathering lists all files in `blueprints/`, then reads `blueprints/00_project.md` for project-wide work and the relevant module/file blueprint files in full.
...
- Blueprint files are not ordinary summaries. Their center thought is that an AI should be able to recreate a module with equivalent behavior from the blueprint.
