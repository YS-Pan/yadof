...
- HTCondor submit file: `job.sub` with `executable` set to the configured worker Python executable, `arguments = workflow.py`, `transfer_executable = False`, and sandboxed Windows profile/temp environment variables. `transfer_output_files` is intentionally omitted so HTCondor returns generated files, including PyAEDT-created `batch.log` when present, without holding the job if optional files are absent.
