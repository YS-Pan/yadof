...
- Local and HTCondor controls are simple strings, numbers, and booleans such as `EVALUATION_TIMEOUT_SEC`, `LOCAL_EVALUATION_MAX_WORKERS`, `HTCONDOR_SUBMIT_EXE`, `HTCONDOR_EXECUTABLE_MODE`, `HTCONDOR_PYTHON_EXE`, `HTCONDOR_REQUEST_CPUS`, `HTCONDOR_REQUEST_MEMORY`, `HTCONDOR_ENVIRONMENT`, `HTCONDOR_LOAD_PROFILE`, `HTCONDOR_RUN_AS_OWNER`, and `HTCONDOR_REQUIREMENTS`.
...
- `HTCONDOR_EXECUTABLE_MODE` defaults to `"workflow"`, so generated submit files use direct `workflow.py` submission with `transfer_executable = True`. This follows the Windows HTCondor debug reference and avoids the fragile absolute-interpreter pattern.
- `HTCONDOR_PYTHON_EXE` is retained for the explicit `"python"` executable mode fallback. If an executable path is used in that mode, it must be valid on every worker that can match the job.
