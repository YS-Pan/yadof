...
PLAN_PATH = V3_ROOT / "validation_plan.json"
...
def _conditional_named_samples(
    template_sample: StructuredRawDataSample,
    samples: Sequence[Sequence[Mapping[str, object]]],
) -> tuple[StructuredRawDataSample, ...]:
    filenames = tuple(item.filename for item in template_sample.items)
    if any(len(row) != len(filenames) for row in samples):
        raise ValueError("conditional-INR prediction item count drifted")
    return tuple(
        StructuredRawDataSample.from_items(
            tuple(
                NamedRawDataItem(filename, dict(payload))
                for filename, payload in zip(filenames, row)
            )
        )
        for row in samples
    )
...
def _conditional_cell(
    *,
    ...
    with _ResourceMonitor() as monitor:
        ...
        predicted = _conditional_named_samples(data.samples[0], raw_predicted)
    scalers = fit_scalers(
