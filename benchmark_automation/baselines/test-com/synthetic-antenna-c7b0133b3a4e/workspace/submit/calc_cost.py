...
# Fixed calibration anchors for the synthetic response model.  They intentionally
# span its useful physical range so random and optimized designs remain ordered
# well inside the algebraic soft-cost tails.
S11_RESONANCE_GOAL_DB = -8.0
S11_RESONANCE_WORST_DB = -5.5
BEAM_GAIN_GOAL_DB = -1.5
BEAM_GAIN_WORST_DB = -4.0
BACK_LOBE_GOAL_DB = -22.5
BACK_LOBE_WORST_DB = -20.5
AXIAL_RATIO_GOAL_DB = 8.0
AXIAL_RATIO_WORST_DB = 16.5
...
def _axial_value(view: RawDataView) -> float:
    ...
    theta = view.nearest_index(
        "Theta", AXIAL_RATIO_TARGET_THETA_BY_STATE[_pin_state(view)],
        ANGLE_TOLERANCE_DEG, period=360.0, converter=angle_to_degrees,
    )
    frequency = view.nearest_index("Freq", TARGET_FREQ_GHZ, 0.02)
