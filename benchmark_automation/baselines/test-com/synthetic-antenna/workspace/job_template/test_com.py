...
def _antenna_objective_positions(x: np.ndarray) -> np.ndarray:
    """Return a rugged four-objective DTLZ-like antenna design position.

    The first three variables select a point on a four-objective Pareto direction.
    The remaining seventeen variables share a non-separable, multimodal design
    loss. Its unique global basin is centered at 0.5, while the periodic term
    creates many inferior basins that deliberately require a longer search.
    """

    z = np.asarray(x[3:], dtype=np.float64) - 0.5
    mixed = 0.72 * z + 0.24 * np.roll(z, 1) - 0.16 * np.roll(z, 4)
    rugged = np.mean(
        mixed * mixed + 0.075 * (1.0 - np.cos(4.0 * math.pi * mixed))
    )
    shared_loss = 3.2 * float(rugged)

    angles = 0.5 * math.pi * np.asarray(x[:3], dtype=np.float64)
    cosines = np.cos(angles)
    sines = np.sin(angles)
    direction = np.asarray(
        (
            cosines[0] * cosines[1] * cosines[2],
            cosines[0] * cosines[1] * sines[2],
            cosines[0] * sines[1],
            sines[0],
        ),
        dtype=np.float64,
    )
    return direction + shared_loss
...
def _s11_trace(
    h: np.ndarray, pin_state: int, objective_position: float
) -> np.ndarray:
    freq = HFSS_FREQ_AXIS
    ...
    desired_resonance_db = -8.95 + 3.75 * float(objective_position)
    trace += desired_resonance_db - float(np.min(trace))
...
def _gain_grid(
    h: np.ndarray,
    pin_state: int,
    beam_objective_position: float,
    back_lobe_objective_position: float,
) -> np.ndarray:
    ...
    phi_index = int(np.argmin(_periodic_delta(HFSS_PHI_AXIS, TARGET_PHI_DEG)))
    theta_index = int(np.argmin(np.abs(HFSS_THETA_AXIS - theta_target)))
    desired_beam_gain_db = -0.4 - 5.0 * float(beam_objective_position)
    beam_delta = desired_beam_gain_db - float(grid[0, phi_index, theta_index])
    beam_shape = _periodic_gaussian(phi, TARGET_PHI_DEG, phi_width) * _gaussian(
        theta, theta_target, theta_width
    )
    grid += beam_delta * beam_shape

    back_mask = np.abs(HFSS_THETA_AXIS) >= 150.0
    desired_back_lobe_db = -23.95 + 3.75 * float(back_lobe_objective_position)
    observed_back_lobe_db = float(np.max(grid[0, phi_index, back_mask]))
    back_delta = desired_back_lobe_db - observed_back_lobe_db
    back_taper = np.clip((np.abs(theta) - 145.0) / 5.0, 0.0, 1.0)
    back_phi_shape = _periodic_gaussian(phi, TARGET_PHI_DEG, 36.0)
    grid += back_delta * back_phi_shape * back_taper
    return np.asarray(grid, dtype=np.float64)


def _axial_ratio_grid(
    h: np.ndarray, pin_state: int, objective_position: float
) -> np.ndarray:
    ...
    frequency_index = int(np.argmin(np.abs(HFSS_FREQ_AXIS - 2.44)))
    phi_index = int(np.argmin(_periodic_delta(HFSS_PHI_AXIS, TARGET_PHI_DEG)))
    theta_index = int(np.argmin(np.abs(HFSS_THETA_AXIS - theta_target)))
    desired_axial_ratio_db = 5.5 + 12.5 * float(objective_position)
    grid += desired_axial_ratio_db - float(
        grid[frequency_index, phi_index, theta_index]
    )
...
def _hfss_like_outputs(
    h: np.ndarray, objective_positions: np.ndarray
    ...
    for pin_state in PIN_STATES:
        ...
        blocks[s11_name] = _hfss_trace_block(
            s11_name,
            _s11_trace(h, pin_state, float(objective_positions[0])),
            pin_state=pin_state,
            quantity="s11",
            expression="dB(S(1,1))",
        )
        blocks[gain_name] = _hfss_grid_block(
            ...
            _gain_grid(
                h,
                pin_state,
                float(objective_positions[1]),
                float(objective_positions[2]),
            ),
            pin_state=pin_state,
        ...
        blocks[axial_name] = _hfss_grid_block(
            axial_name,
            _axial_ratio_grid(h, pin_state, float(objective_positions[3])),
            pin_state=pin_state,
...
def evaluate_raw_data(
    ...
    *,
    ...
    if profile_key in {"hfss", "hfss_like", "newchoke"}:
        return _hfss_like_outputs(h, _antenna_objective_positions(x))
