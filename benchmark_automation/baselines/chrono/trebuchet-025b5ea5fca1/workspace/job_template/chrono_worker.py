...
def run_task_model(
    ...
    *,
    ...
    ground_material = chrono.ChContactMaterialSMC()
    ground_material.SetYoungModulus(2.0e7)
    ground_material.SetPoissonRatio(0.30)
    ground_material.SetFriction(0.60)
    ground_material.SetRestitution(0.05)

    ball_material = chrono.ChContactMaterialSMC()
    ball_material.SetYoungModulus(5.0e6)
    ball_material.SetPoissonRatio(0.35)
    ...
    # tension lifts it.  SMC contact alone is compliant enough that the exact
    # distance constraint can pull this light ball visibly through the floor.
    ...
    while float(system.GetChTime()) < SIMULATION_DURATION_S - 0.5 * TIME_STEP_S:
        ...
        current_arm_tip, _, current_hinge, current_cw, arm_direction, hanger_direction = current_directions()
        ...
        if (
            ...
            # one-step SMC penetration from becoming part of the launch state.
            ball.SetPos(
                _vector(
                    chrono,
...
def simulate(request: Mapping[str, object], rawdata_dir: Path) -> Mapping[str, object]:
    ...
    total_time = np.asarray(result["total_time_s"], dtype=np.float64)
    summary = np.asarray(result["summary"], dtype=np.float64)
    release_kinematics = np.asarray(
        result["release_kinematics"],
        dtype=np.float64,
    )
    ...
    total_time_metadata = {
        "schema_version": 1,
        "rawdata_name": TOTAL_TIME_RAWDATA_NAME,
        "shape": list(total_time.shape),
        "unit": "s",
    ...
    _save_npz(
        rawdata_dir,
        f"{TOTAL_TIME_RAWDATA_NAME}.npz",
        values=total_time,
        metadata=np.asarray(
            json.dumps(total_time_metadata, separators=(",", ":"))
        ),
    )

    summary_metadata = {
        "schema_version": 1,
        "rawdata_name": RELEASE_SUMMARY_RAWDATA_NAME,
        "shape": list(summary.shape),
        "quantity_names": list(RELEASE_SUMMARY_NAMES),
        "quantity_units": list(RELEASE_SUMMARY_UNITS),
        "model": MODEL_NAME,
    }
    _save_npz(
        rawdata_dir,
        f"{RELEASE_SUMMARY_RAWDATA_NAME}.npz",
        values=summary,
        metadata=np.asarray(json.dumps(summary_metadata, separators=(",", ":"))),
    )

    kinematics_metadata = {
        "schema_version": 1,
        "rawdata_name": RELEASE_KINEMATICS_RAWDATA_NAME,
        "shape": list(release_kinematics.shape),
        "axes": [
            {
                "index": 0,
                "size": int(release_kinematics.shape[0]),
                "name": "release_phase",
                "values_key": "axis_release_phase",
            },
            {
                "index": 1,
                "size": int(release_kinematics.shape[1]),
                "name": "channel",
                "values_key": "axis_channel",
            },
        ],
        "channel_names": list(RELEASE_KINEMATICS_CHANNEL_NAMES),
        "channel_units": list(RELEASE_KINEMATICS_CHANNEL_UNITS),
        "time_parameterization": "physical_time_s = release_phase * total_time_s",
        "model": MODEL_NAME,
    }
    _save_npz(
        rawdata_dir,
        f"{RELEASE_KINEMATICS_RAWDATA_NAME}.npz",
        values=release_kinematics,
        axis_release_phase=phase_axis,
        axis_channel=np.arange(release_kinematics.shape[1], dtype=np.float64),
        metadata=np.asarray(
            json.dumps(kinematics_metadata, separators=(",", ":"))
        ),
    )

    stress_metadata = {
        "schema_version": 1,
        "rawdata_name": STRESS_HISTORY_RAWDATA_NAME,
        "shape": list(stress.shape),
        "axes": [
            {
                "index": 0,
                "size": int(stress.shape[0]),
                "name": "release_phase",
                "values_key": "axis_release_phase",
            },
            {
                "index": 1,
                "size": int(stress.shape[1]),
                "name": "channel",
                "values_key": "axis_channel",
            },
        ],
        "channel_names": list(STRESS_HISTORY_CHANNEL_NAMES),
        "channel_units": list(STRESS_HISTORY_CHANNEL_UNITS),
    ...
    _save_npz(
        rawdata_dir,
        f"{STRESS_HISTORY_RAWDATA_NAME}.npz",
        values=stress,
        axis_release_phase=phase_axis,
        axis_channel=np.arange(stress.shape[1], dtype=np.float64),
        metadata=np.asarray(json.dumps(stress_metadata, separators=(",", ":"))),
    )
    ...
    diagnostics.update(
        {
            ...
            "rawdata_shapes": {
                TOTAL_TIME_RAWDATA_NAME: list(total_time.shape),
                RELEASE_SUMMARY_RAWDATA_NAME: list(summary.shape),
                RELEASE_KINEMATICS_RAWDATA_NAME: list(release_kinematics.shape),
                STRESS_HISTORY_RAWDATA_NAME: list(stress.shape),
            },
        }
    )
    return diagnostics
