...
_WINDOWS_MAX_CURRENT_DIRECTORY_CHARS = 259
...
def run_pychrono(
    ...
    *,
    ...
    try:
        launch_scratch, launch_alias = _launch_scratch(scratch)
        launch_request_path = launch_scratch / request_path.name
        launch_result_path = launch_scratch / result_path.name
        _write_bytes_atomic(
            request_path,
            _encode_json(
        ...
        launch_environment = _child_environment(
            environment, scratch=launch_scratch, interpreter=interpreter_path
        )
        ...
        cleanup_error = _remove_scratch(scratch, launch_alias=launch_alias)
        if cleanup_error:
...
def _launch_scratch(scratch: Path) -> tuple[Path, Path | None]:
    """Return the child-visible scratch and its optional Windows junction alias."""
    ...
    try:
        alias = Path(tempfile.mkdtemp(prefix="yadof-pychrono-launch-")).resolve()
        if len(str(alias)) > _WINDOWS_MAX_CURRENT_DIRECTORY_CHARS:
            raise OSError(
                "the Windows temporary directory is too long for a child launch alias"
            )
        _set_windows_junction(alias, scratch)
        return alias, alias
    except OSError as exc:
        if alias is not None:
            try:
                ...
                pass
        if len(str(scratch)) <= _WINDOWS_MAX_CURRENT_DIRECTORY_CHARS:
            return scratch, None
        raise PyChronoError(
            "launch_failed",
            "could not create a short Windows launch alias for the candidate "
            f"scratch: {exc}",
        ) from exc


def _set_windows_junction(alias: Path, target: Path) -> None:
    """Turn one reserved empty directory into a junction to a local target."""
    ...
    if str(resolved_target).startswith("\\\\"):
        raise OSError("Windows launch junctions require a local scratch target")
    if not alias.is_dir() or any(alias.iterdir()):
        raise OSError("Windows launch alias must be an empty directory")

    substitute_name = "\\??\\" + str(resolved_target)
    ...
    if len(reparse_data) > 16 * 1024:
        raise OSError("candidate scratch exceeds the Windows junction data limit")
...
def _remove_scratch(
    scratch: Path, *, launch_alias: Path | None = None
) -> str | None:
    errors: list[str] = []
    if launch_alias is not None:
        try:
            launch_alias.rmdir()
        except OSError as exc:
            errors.append(f"launch alias {launch_alias}: {exc}")
    try:
        shutil.rmtree(scratch)
    except OSError as exc:
        errors.append(f"candidate scratch {scratch}: {exc}")
    if launch_alias is not None and os.path.lexists(launch_alias):
        try:
            launch_alias.rmdir()
        except OSError as exc:
            errors.append(f"dangling launch alias {launch_alias}: {exc}")
