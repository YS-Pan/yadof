...
TOTAL_TIME_RAWDATA_FIELD = (
    "trebuchet_total_time",
    "total_time_s",
    "s",
)

# Every semantic quantity is a separate rawData field.  In particular, the
# phase histories below are independent 1-D curves rather than columns joined
# into an artificial 2-D channel image for the surrogate.
RELEASE_SUMMARY_RAWDATA_FIELDS = (
    ("trebuchet_loaded_max_height", "loaded_max_height_m", "m"),
    ("trebuchet_minimum_ball_center_z", "minimum_ball_center_z_m", "m"),
    ("trebuchet_trigger_time", "trigger_time_s", "s"),
    ("trebuchet_released_flag", "released_flag", "1"),
    (
        "trebuchet_release_sling_arm_rotation",
        "release_sling_arm_rotation_deg",
        "deg",
    ),
    (
        "trebuchet_minimum_arm_ground_clearance",
        "minimum_arm_ground_clearance_m",
        "m",
    ),
    (
        "trebuchet_minimum_hanger_ground_clearance",
        "minimum_hanger_ground_clearance_m",
        "m",
    ),
    (
        "trebuchet_minimum_counterweight_ground_clearance",
...
RELEASE_KINEMATICS_RAWDATA_FIELDS = (
    ("trebuchet_ball_x", "ball_x_m", "m"),
    ("trebuchet_ball_z", "ball_z_m", "m"),
    ("trebuchet_ball_vx", "ball_vx_mps", "m/s"),
    ("trebuchet_ball_vz", "ball_vz_mps", "m/s"),
)

STRESS_HISTORY_RAWDATA_FIELDS = (
    (
        "trebuchet_arm_combined_normal_stress",
        "arm_combined_normal_stress_mpa",
        "MPa",
    ),
    (
        "trebuchet_hanger_combined_normal_stress",
        "hanger_combined_normal_stress_mpa",
        "MPa",
    ),
    (
        "trebuchet_peak_strength_utilization",
...
RELEASE_SUMMARY_NAMES = tuple(field[1] for field in RELEASE_SUMMARY_RAWDATA_FIELDS)
RELEASE_SUMMARY_UNITS = tuple(field[2] for field in RELEASE_SUMMARY_RAWDATA_FIELDS)
RELEASE_KINEMATICS_CHANNEL_NAMES = tuple(
    field[1] for field in RELEASE_KINEMATICS_RAWDATA_FIELDS
)
RELEASE_KINEMATICS_CHANNEL_UNITS = tuple(
    field[2] for field in RELEASE_KINEMATICS_RAWDATA_FIELDS
)
STRESS_HISTORY_CHANNEL_NAMES = tuple(
    field[1] for field in STRESS_HISTORY_RAWDATA_FIELDS
)
STRESS_HISTORY_CHANNEL_UNITS = tuple(
    field[2] for field in STRESS_HISTORY_RAWDATA_FIELDS
)
...
def initial_geometry(spec: ModelSpec) -> dict[str, tuple[float, float] | float]:
    ...
    arm_ground_clearance = min(arm_tip[1], hanger_hinge[1]) - arm_section_vertical
    ...
    hanger_ground_clearance = (
        min(hanger_hinge[1], counterweight_center[1]) - hanger_section_vertical
    )
    ...
    counterweight_ground_clearance = counterweight_center[1] - cw_half_vertical
    ...
    return {
        ...
        "loaded_arm_ground_clearance_m": arm_ground_clearance,
        "loaded_hanger_ground_clearance_m": hanger_ground_clearance,
        "loaded_counterweight_ground_clearance_m": (
            counterweight_ground_clearance
        ),
    }
