...
from .modeling import MODEL_NAME, save_model_bundle
...
def schema_payload(schema: HierarchicalSchema | None) -> dict[str, object]:
    ...
    return {
        ...
        "rawdata_item_count": len(schema.layouts),
        "flat_dim": offset,
        "modeled_slots": slots,
        "layouts": [layout.as_dict() for layout in schema.layouts],
        "groups": [
            [list(selector) for selector in group] for group in schema.groups
        ],
    }
    ...
    payload = {
        ...
        "axis_signatures": (
            ...
            if schema is None
                ...
                [_array_signature(values) for values in layout.axis_values]
...
def validate_manifest_identity(payload: object) -> dict[str, object]:
    ...
    if payload.get("quality_policy") is not None and not isinstance(
        payload.get("quality_policy"), Mapping
    ):
...
def _checkpoint_payload(
    state: HierarchicalState, checkpoint_root: Path
) -> dict[str, object]:
    ...
    return {
        ...
        "model_path": state.bundle_path.name,
        "scaler_path": "field_scalers.npz",
        "artifact_dir": state.artifact_dir.relative_to(
            checkpoint_root
        ).as_posix(),
        ...
        "train_cfg": asdict(state.train_cfg),
        "quality_policy": (
            None
        ...
        "note": (
            ...
            "Predictor-member spread is finite and uncalibrated."
        ),
    }


def write_checkpoint(
    state: HierarchicalState, *, staged_artifact_dir: Path
) -> None:
    ...
    save_model_bundle(
        staging / state.bundle_path.name,
        model=state.model,
