...
from .._shared.finite_members import seeded_member_indices as _seeded_member_indices
...
class ConditionalINRRawDataSampler:
    ...
    def __init__(self, state: SurrogateState, schema_template: RawDataSchemaTemplate, selectors_by_item: tuple[RawDataFieldSelector, ...], member_indices: tuple[int, ...], *, draw_count: int, seed: int, strategy_signature: str) -> None:
        self._state = state
        self._schema_template = schema_template
        self._selectors_by_item = selectors_by_item
        self._member_indices = member_indices
        draw_ids = tuple((f'conditional-inr-seed-{int(seed)}-draw-{index:06d}' for index in range(int(draw_count))))
        self._diagnostics = RawDataPosteriorDiagnostics(posterior_kind='empirical_ensemble', requested_draw_count=int(draw_count), support_kind=SUPPORT_FINITE, unique_support=len(member_list(state.model)), seed=int(seed), draw_ids=draw_ids, draw_sources=tuple((f'conditional-inr-member-{member_index:04d}' for member_index in member_indices)), schema_signature=schema_template.signature, state_signature=str(state.state_signature), strategy_signature=str(strategy_signature), approximate=True, limitations=('finite empirical ensemble support; repeated draws add no support', 'conditional-INR member spread is not a calibrated posterior', 'full stored-grid rawData reconstruction only', 'observation noise is not included'), field_selectors=schema_template.field_selectors, observation_noise_included=False)
    ...
    def predict(self, population: Sequence[Sequence[float]]) -> MaterializedRawDataPosterior:
        rows = tuple((tuple((float(value) for value in row)) for row in population))
        cache: dict[tuple[int, tuple[float, ...]], StructuredRawDataSample | _FailedPrediction] = {}
        draws = []
        retained_failures: list[Mapping[str, object]] = []
        failure_count = 0
        complete_sources: set[str] = set()
        for draw_index, (draw_id, member_index, source) in enumerate(zip(self._diagnostics.draw_ids, self._member_indices, self._diagnostics.draw_sources)):
            samples = []
            ...
            for candidate_index, row in enumerate(rows):
                ...
                if isinstance(result, _FailedPrediction):
                    ...
                    if len(retained_failures) < _MAX_RETAINED_FAILURES:
                        retained_failures.append({'error_type': result.error_type, 'draw_id': draw_id, 'draw_index': draw_index, 'draw_source': source, 'member_index': member_index, 'candidate_index': candidate_index, 'message': result.message})
                else:
                    samples.append(result)
            if draw_complete:
                complete_sources.add(source)
            draws.append(RawDataFunctionDraw(draw_id, tuple(samples)))
        diagnostics = self._diagnostics.for_prediction(len(rows), prediction_failure_count=failure_count, retained_prediction_failures=retained_failures, effective_unique_support=len(complete_sources))
        return MaterializedRawDataPosterior(rows, tuple(draws), diagnostics)

    def _predict_one(self, member_index: int, row: tuple[float, ...]) -> StructuredRawDataSample | _FailedPrediction:
        try:
            if len(row) != len(self._state.parameter_names):
                raise ValueError('normalized candidate width does not match the trained state')
            ...
            predicted = runtime._predict_selected_member_flat(self._state, x, member_index)
            if predicted.shape != (1, int(self._state.schema.flat_dim)):
                raise ValueError('conditional-INR member prediction returned an unexpected shape')
            return _reconstruct_member_sample(self._schema_template, self._selectors_by_item, self._state.schema, predicted[0])
...
def make_rawdata_sampler(context, *, component, draw_count: int, seed: int) -> ConditionalINRRawDataSampler:
    ...
    if state.strategy_signature != strategy_signature:
        raise RuntimeError('conditional-INR posterior state belongs to a different strategy namespace')
...
def _schema_template_from_context(context, schema: RawDataSchema) -> tuple[RawDataSchemaTemplate, tuple[RawDataFieldSelector, ...]]:
    ...
    for job_name, items in evidence:
        ...
        try:
            state_items = []
            selectors = []
            for item_index, (evidence_item, payload) in enumerate(zip(items, schema.templates)):
                ...
                if evidence_key != state_key:
                    raise ValueError(f'item {item_index} main key changed from {state_key!r} to {evidence_key!r}')
            ...
            for slot in schema.modeled_slots:
                if slot.key != selectors[slot.item_index][1]:
                    raise ValueError('posterior reconstruction requires all modeled slots to be resolved rawData main arrays; axes and metadata stay frozen')
    ...
    raise RuntimeError(f'no completed named rawData sample is compatible with the trained conditional-INR schema ({detail})')

def _reconstruct_member_sample(template: RawDataSchemaTemplate, selectors_by_item: tuple[RawDataFieldSelector, ...], schema: RawDataSchema, flat: np.ndarray) -> StructuredRawDataSample:
    arrays = {field.selector: np.asarray(field.payload[field.main_key]).copy() for field in template.fields}
