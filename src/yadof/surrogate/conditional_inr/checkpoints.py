...
def semantic_state_signature(*, strategy_signature: str, parameter_names: tuple[str, ...], parameter_definition_signature: Mapping[str, object], schema: RawDataSchema | None, train_cfg: INRTrainConfig | None, torch_version: str | None=None) -> str:
    payload = {'strategy_signature': str(strategy_signature), 'surrogate_method': SURROGATE_METHOD, 'training_policy': TRAINING_POLICY, 'parameter_names': list(parameter_names), 'parameter_definition_signature': dict(parameter_definition_signature), 'schema': schema_payload(schema), 'coord_table': _array_signature(np.zeros((0, 3), dtype=np.float32) if schema is None else schema.coord_table), 'field_ids': _array_signature(np.zeros((0,), dtype=np.int64) if schema is None else schema.field_ids), 'train_cfg': None if train_cfg is None else asdict(train_cfg), 'torch_version': str(torch.__version__) if torch_version is None else str(torch_version)}
...
def validate_manifest_identity(payload: object) -> dict[str, object]:
    ...
    if str(payload['run_namespace']) != run_namespace_for_signature(strategy_signature):
        raise ValueError('surrogate checkpoint run namespace does not match its signature')
    ...
    if not isinstance(payload['parameter_definition_signature'], Mapping):
        raise ValueError('surrogate checkpoint parameter definition signature must be an object')
...
def resolve_artifact_dir(checkpoint_dir: Path, payload: object) -> Path:
    ...
    if relative.is_absolute() or '..' in relative.parts:
        raise ValueError('surrogate checkpoint artifact path must stay below its checkpoint root')
    resolved = (root / relative).resolve()
    if not resolved.is_relative_to(root):
        raise ValueError('surrogate checkpoint artifact path escapes its checkpoint root')
    expected_parent = (root / 'runs' / str(manifest['run_namespace']) / 'components' / str(manifest['component_namespace'])).resolve()
    if resolved.parent != expected_parent:
        raise ValueError('surrogate checkpoint artifact is outside its declared namespace')
    expected_name = f"generation_{int(manifest['generation_index']):04d}_{manifest['publication_id']}"
    if resolved.name != expected_name:
        raise ValueError('surrogate checkpoint artifact name does not match its publication')
    return resolved
...
def resolve_namespace_manifest_path(checkpoint_dir: Path, payload: object) -> Path:
    ...
    if relative.is_absolute() or '..' in relative.parts:
        raise ValueError('surrogate namespace manifest path must stay below its checkpoint root')
    resolved = (root / relative).resolve()
    expected_parent = (root / 'runs' / str(manifest['run_namespace']) / 'components' / str(manifest['component_namespace'])).resolve()
    ...
    if resolved.parent != expected_parent or resolved.name != expected_name:
        raise ValueError('surrogate namespace manifest is outside its declared namespace')
    return resolved
...
def schema_payload(schema: RawDataSchema | None) -> dict[str, object]:
    ...
    return {'rawdata_item_count': len(schema.templates), 'flat_dim': int(schema.flat_dim), 'query_count': int(schema.coord_table.shape[0]), 'n_fields': int(schema.n_fields), 'modeled_slots': [{'item_index': int(slot.item_index), 'key': slot.key, 'shape': list(slot.shape), 'dtype': slot.dtype, 'start': int(slot.start), 'end': int(slot.end), 'field_id': int(slot.field_id)} for slot in schema.modeled_slots]}

def _write_auxiliary_artifact(state: SurrogateState, staging_dir: Path) -> None:
    arrays: dict[str, np.ndarray] = {'schema_flat_dim': np.asarray(0 if state.schema is None else state.schema.flat_dim, dtype=np.int64), 'training_sample_count': np.asarray(state.sample_count, dtype=np.int64)}
    ...
    if state.scaler is not None:
        arrays['target_mean'] = np.ascontiguousarray(state.scaler.mean, dtype=np.float64)
        arrays['target_scale'] = np.ascontiguousarray(state.scaler.scale, dtype=np.float64)
    np.savez_compressed(staging_dir / state.model_path.name, **arrays)

def _checkpoint_payload(state: SurrogateState, checkpoint_root: Path) -> dict[str, object]:
    ...
    if not manifest_stem.startswith(prefix):
        raise ValueError('surrogate namespace manifest does not encode its generation')
    publication_id = manifest_stem[len(prefix):]
    return {'surrogate_method': SURROGATE_METHOD, 'training_policy': TRAINING_POLICY, 'strategy_signature': state.strategy_signature, 'state_signature': state.state_signature, 'run_namespace': state.run_namespace, 'component_namespace': state.component_namespace, 'publication_id': publication_id, 'torch_version': str(torch.__version__), 'generation_index': int(state.generation_index), 'sample_count': int(state.sample_count), 'parameter_names': list(state.parameter_names), 'parameter_definition_signature': dict(state.parameter_definition_signature), 'model': state.model_name, 'member_count': int(state.train_history.get('member_count', 0)), 'model_path': state.model_path.name, 'artifact_dir': state.artifact_dir.relative_to(checkpoint_root).as_posix(), 'namespace_manifest': state.namespace_manifest_path.relative_to(checkpoint_root).as_posix(), 'schema': schema_payload(state.schema), 'train_cfg': None if state.train_cfg is None else asdict(state.train_cfg), 'train_history': state.train_history, 'note': 'The real-field-balanced conditional INR predicts full rawData; its linear decoder predicts per-query standard scores and current costs are derived through submit/calc_cost.py. Ensemble spread is diagnostic, not calibrated confidence.'}

def write_checkpoint(state: SurrogateState, *, staged_artifact_dir: Path) -> None:
    ...
    if member_count > 0:
        required = [staging / 'inr_meta.json'] + [staging / f'member_{index:03d}.pt' for index in range(member_count)]
        missing = [path.name for path in required if not path.is_file()]
        if missing:
            raise ValueError('checkpoint staging is missing model artifact(s): ' + ', '.join(missing))
