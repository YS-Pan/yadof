...
| `terminal report` | A complete stdout-only `summary`, `audit`, or `inspect` result below `yadof view surrogate`. It identifies strategy/run/component scope, is human-readable text or schema-versioned JSON, uses stderr for optional progress/errors, and is never persisted unless inspect receives explicit output. |
...
| `case inspection` | One deterministic headless comparison of an exact checkpoint with one exact completed real result and one resolved 0D/1D/2D rawData query. It reuses `predict_one`, bounds inline arrays, preserves off-grid truth absence, and never mutates workspace evidence. |
| `inspection evidence directory` | Explicit collision-free `inspect --output` destination containing full selected NPZ arrays, an Agg PNG, optional one-dimensional CSV, and a manifest published last with relative paths, byte sizes, and SHA-256 digests. |
