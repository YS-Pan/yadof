...
- A **viewer user** selects a compatible yadof workspace, chooses checkpoints and
  real individuals, adjusts parameters, starts or stops audits, and interprets
  plots.
...
- **PyTorch** executes checkpoint inference on the configured CPU, CUDA, or XPU
  device.
- **Tkinter** owns desktop interaction and widget state.
...
The viewer starts at `yadof view surrogate [--workspace PATH]` or the equivalent
`python -m yadof.tools.surrogate_viewer` module entry. It is installed below
`yadof.tools`, but remains outside core optimization/evaluation imports. The
selected workspace is an explicit input or an explicit GUI selection, not an
inferred "current project".
