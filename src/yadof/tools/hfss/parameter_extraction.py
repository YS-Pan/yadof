...
def _backup_parameter_file(
    parameter_file: Path, history_dir: Path, *, timestamp: str
) -> Path | None:
    if not parameter_file.exists():
        return None

    history_dir.mkdir(parents=True, exist_ok=True)
    dest = history_dir / f"parameters_constraints_{timestamp}.py"

    # Ensure uniqueness
    if dest.exists():
        i = 1
        while True:
            cand = history_dir / f"parameters_constraints_{timestamp}_{i}.py"
            if not cand.exists():
                dest = cand
                break
            i += 1

    shutil.copy2(parameter_file, dest)
    return dest


class ParameterExtractionConfirmationRequired(RuntimeError):
    """Raised when replacing parameters lacks explicit confirmation."""
...
def extract_parameters(
    ...
    *,
    ...
    """Extract HFSS variables into one workspace's current parameter file."""

    config = load_config(workspace)
    parameter_file = config.workspace.job_template_dir / "parameters_constraints.py"
    if parameter_file.exists() and not confirm:
        raise ParameterExtractionConfirmationRequired(
            "parameter extraction replaces parameters_constraints.py; "
            "pass confirm=True or use CLI --yes"
        )
    if project is None:
        project_path = _scan_single_aedt_file(config.workspace.job_template_dir)
    else:
        selected = Path(project).expanduser()
        if not selected.is_absolute():
            selected = config.workspace.root / selected
        project_path = selected.resolve()
    if not project_path.exists():
        raise FileNotFoundError(project_path)

    non_graphical = not bool(graphical)
    quiet = not bool(verbose)

    # Read the current file as the formatting template before replacing it.
    template_text: str | None = None
    if parameter_file.exists():
        template_text = parameter_file.read_text(encoding="utf-8")

    # ---- Primary path: parse .aedt file directly (supports discrete ranges) ----
    params: list[OptParam] | None = None
    used_direct_parser = False
    try:
        params = _collect_parameters_from_aedt_file(project_path)
        if params:
            used_direct_parser = True
        else:
    ...
    if not params:
        ...
        try:
            hfss = _open_hfss_project(
                project_path, design_name=design,
    ...
    tmp_path = parameter_file.with_name(
        f".{parameter_file.name}.tmp_{os.getpid()}"
    )
    try:
        tmp_path.write_text(new_text, encoding="utf-8", newline="\n")

        archived = _backup_parameter_file(
            parameter_file,
            config.workspace.tool_output_dir / "parameter_history",
            timestamp=timestamp,
        )
        os.replace(str(tmp_path), str(parameter_file))
        return ParameterExtractionResult(
            project_path=project_path,
            parameter_count=len(params),
            parameter_file=parameter_file,
            backup_file=archived,
            used_direct_parser=used_direct_parser,
        )
    finally:
        try:
            if tmp_path.exists():
...
__all__ = [
    "ParameterExtractionConfirmationRequired",
    "ParameterExtractionResult",
    "extract_parameters",
]
