"""Validation and generic views for task rawData ``.npz`` files."""
...
class RawDataView:
    ...
    def axis(self, name: str) -> tuple[np.ndarray, str]:
        ...
        return (
            self.axis_values.get(name, np.asarray([], dtype=float)),
            self.axis_units.get(name, ""),
        )

    def axis_coordinates(
        self, name: str, converter: AxisConverter | None = None
    ) -> np.ndarray:
    ...
    def select(
        ...
        *,
        ...
        index = self.nearest_index(
            name, target, tolerance, period=period, converter=converter
        )
        return RawDataView(
            item=self.item,
            ...
            axis_values={
                key: value for key, value in self.axis_values.items() if key != name
            },
            axis_units={
                key: value for key, value in self.axis_units.items() if key != name
            },
        )
    ...
    def range_indices(
        ...
        *,
        ...
        return np.flatnonzero(
            np.isfinite(coordinates) & (coordinates >= lo) & (coordinates <= hi)
        )
...
def combine_2d_curves(
    ...
    for item in items:
        ...
        if x.size != y.size:
            raise ValueError(
                f"{item.name} {axis_name}/data size mismatch: {x.size} != {y.size}"
            )
        x_chunks.append(x)
        y_chunks.append(y)
    if not x_chunks:
...
def angle_to_degrees(values: np.ndarray, unit: str) -> np.ndarray:
    ...
    if unit_text.startswith("rad") or (
        not unit_text
        and values.size
        and float(np.max(np.abs(values))) <= 2.0 * math.pi + 1e-9
    ):
...
def parse_metadata(raw: object) -> dict[str, object]:
    ...
    raise RawDataContractError(
        f"unsupported rawData metadata type: {type(raw).__name__}"
    )
...
def validate_rawdata_item(
    item: RawDataItem, *, require_metadata: bool = True
) -> dict[str, object]:
    loaded = load_rawdata_item(item)
    metadata = metadata_from_item(loaded)
    if require_metadata and not metadata:
        raise RawDataContractError(
            "rawData item must contain metadata", error_type="missing_metadata"
        )
    if not metadata:
        return loaded
...
def validate_rawdata_directory(rawdata_dir: str | Path) -> tuple[Path, ...]:
    ...
    if subdirs:
        names = ", ".join(
            path.name for path in sorted(subdirs, key=lambda path: path.name.lower())
        )
        raise RawDataContractError(
...
def _axis_descriptor(
    metadata: Mapping[str, object], axis_index: int
) -> Mapping[str, object]:
    axes = metadata.get("axes")
    if (
        not isinstance(axes, Sequence)
        or isinstance(axes, (str, bytes, Mapping))
        or axis_index >= len(axes)
    ):
        return {}
...
def _axis_maps(
    ...
    for axis_index, name in enumerate(axis_names):
        ...
        units[name] = str(
            descriptor.get("unit") or _scalar_text(item.get(unit_key, ""))
        )
    return values, units
...
def _validate_declared_shape(
    item: Mapping[str, object], metadata: Mapping[str, object]
) -> tuple[int, ...]:
    ...
    if actual != expected:
        raise RawDataContractError(
            f"rawData {data_key!r} shape mismatch: metadata {expected}, actual {actual}"
        )
    return expected
...
def _validate_declared_axes(
    ...
    if not isinstance(axes, Sequence) or isinstance(axes, (str, bytes, Mapping)):
        raise RawDataContractError(
            "metadata axes must be a sequence ordered by axis index"
        )
    if len(axes) != len(shape):
        raise RawDataContractError(
            f"metadata axes length {len(axes)} does not match shape rank {len(shape)}"
        )
    for expected_index, descriptor in enumerate(axes):
        ...
        if "index" not in descriptor or "size" not in descriptor:
            raise RawDataContractError(
                "metadata axis descriptors must contain index and size"
            )
        axis_index = _metadata_int(descriptor["index"], "metadata axis index")
        if axis_index != expected_index:
            raise RawDataContractError(
                "metadata axes must be ordered by index; "
                f"expected {expected_index}, got {axis_index}"
            )
        axis_size = _metadata_int(descriptor["size"], "metadata axis size")
        if axis_size != shape[axis_index]:
            raise RawDataContractError(
                f"metadata axis {axis_index} size mismatch: "
                f"metadata {axis_size}, shape {shape[axis_index]}"
            )
        ...
        if not isinstance(values_key, str) or not values_key:
            raise RawDataContractError(
                f"metadata axis {axis_index} values_key must be a non-empty string"
            )
        if values_key not in item:
            raise RawDataContractError(
                f"metadata axis {axis_index} values_key {values_key!r} is missing"
            )
        values = np.asarray(item[values_key])
        if values.ndim == 0 or int(values.shape[0]) != axis_size:
            raise RawDataContractError(
                f"metadata axis {axis_index} values length mismatch: "
                f"expected {axis_size}, got {values.shape}"
            )
