...
def _extract_expression_y(sd: Any, expression: str) -> np.ndarray:
    xy = sd.get_expression_data(expression)
...
def _full_matrix_contract(sd: Any, expression: str) -> dict[str, Any]:
    ...
    table = np.asarray(tables[0][_expression_key(tables[0], expression)], dtype=float)
...
def _export_solution_data_npz(
    ...
    *,
    ...
    setup_sweep_name = _build_setup_sweep_name(Mxw2d, setup, sweep)
    ...
    sd = Mxw2d.post.get_solution_data(
    ...
    data = np.asarray(contract["data"], dtype=float) * float(scale)
    meta = {
...
def set_Mxw2d_temp_directory(Mxw2dApp: Any, path: str | Path | None = None) -> bool:
    temp = Path(path or os.environ.get("TEMP") or os.environ.get("TMP") or "").resolve()
    os.makedirs(temp, exist_ok=True)
    return bool(Mxw2dApp.set_temporary_directory(str(temp)))


def get_desktop_pid(Mxw2dApp: Any) -> int | None:
    seen, queue = set(), [Mxw2dApp]
    while queue:
        obj = queue.pop(0)
...
def set_variables(Mxw2dApp: Any, name_to_value) -> bool:
    ...
    if proj:
        _set_variables_low_level(Mxw2dApp._oproject, "ProjectVariableTab", "ProjectVariables", proj)
    if local:
        _set_variables_low_level(Mxw2dApp._odesign, "LocalVariableTab", "LocalVariables", local)
    return True


def set_para(Mxw2dApp: Any, paraFile: str = "parameters_constraints.py") -> bool:
    return set_variables(Mxw2dApp, _load_parameters_py_value_only(paraFile))
...
def analyze(
    ...
    if analyzeSetup is None:
        setups = Mxw2dApp.get_setups()
        if len(setups) != 1:
            raise RuntimeError(f"Expected 1 setup, found {len(setups)}: {setups}")
        analyzeSetup = setups[0]
    ok = Mxw2dApp.analyze_setup(
        name=analyzeSetup,
        cores=CPUcores,
        tasks=ParallelTasks,
        gpus=allocGPUs,
...
def save_transient(
    ...
    *,
    variations: dict[str, str | list[str]] | None = None,
    primary_sweep_variable: str | None = "Time",
    setup: str | None = None,
    sweep: str | None = None,
    out_dir: str = "rawData",
    scale: float = 1.0,
    ...
    '''
    variations: A dict that represents the calculation range of X-axis.
    Format: {"Time": ["5.4ms:5.9ms"]}, {"Time": ["All"]}

    scale: The unit system of some exported data is not SI.
    In this case please specify "scale" value and data will be multiplied by this value.
    '''
    return _export_solution_data_npz(
        Mxw2dApp,
        expression=expression,
        domain="Sweep",  #PyAEDT docs say it should be "Time" for transient data. That's WRONG.
        report_category="Transient Solution Data",
        variations=variations or {"Time": ["All"]},
...
def solver_exit(
    ...
    *,
    ...
    project_file = str(project_path or getattr(Mxw2dApp, "project_file", "") or "")
    cleanup_targets = _project_cleanup_targets(project_file) if project_file else []
    try:
        if save_project:
            Mxw2dApp.save_project()
    finally:
        try:
            Mxw2dApp.release_desktop()
        finally:
            if cleanup_results:
