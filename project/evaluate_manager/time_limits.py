...
def time_limit_for_job(job: JobSpec) -> HTCondorTimeLimit:
    ...
    configured_seconds = _positive_seconds(
        getattr(project_config, "HTCONDOR_JOB_TIMEOUT_SEC", 60 * 60),
        "HTCONDOR_JOB_TIMEOUT_SEC",
    )
    mode = str(getattr(project_config, "HTCONDOR_JOB_TIMEOUT_MODE", "auto")).strip().lower()
    if mode == "fixed":
    ...
    multiplier = _positive_float(
        getattr(project_config, "HTCONDOR_JOB_TIMEOUT_MULTIPLIER", 2.0),
    ...
    durations, source = _calibration_durations(job, generation_index=generation_index)
    if durations:
        trim_fraction = _fraction(
            getattr(project_config, "HTCONDOR_JOB_TIMEOUT_TRIM_TOP_FRACTION", 0.10),
...
def _calibration_durations(
    ...
    *,
    ...
    records = recorded_data_api.list_records()
    if generation_index == 0:
