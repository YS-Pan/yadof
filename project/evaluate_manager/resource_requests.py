...
def request_for_job(job: JobSpec) -> HTCondorResourceRequest:
    ...
    base_memory_mib = _quantity_as_units(htcondor_request_memory(), _MEMORY_FACTORS_MIB, "HTCONDOR_REQUEST_MEMORY")
    base_disk_kib = _quantity_as_units(htcondor_request_disk(), _DISK_FACTORS_KIB, "HTCONDOR_REQUEST_DISK")
    disk_multiplier = _positive_float(getattr(project_config, "HTCONDOR_REQUEST_DISK_MULTIPLIER", 1.0), "HTCONDOR_REQUEST_DISK_MULTIPLIER")
    memory_mib = base_memory_mib
    disk_kib = _scaled_quantity(base_disk_kib, disk_multiplier)
    source = "configured_default"
    sample_count = 0

    measurements, calibration_source = _calibration_measurements(job)
    if measurements is not None:
        ...
        if memory_values:
            memory_mib = _scaled_quantity(_trimmed_high(memory_values), bootstrap_multiplier)
        if disk_values:
...
def _calibration_measurements(job: JobSpec) -> tuple[tuple[list[float], list[float]] | None, str]:
    if not _as_bool(getattr(project_config, "HTCONDOR_RESOURCE_AUTODETECT_ENABLED", True)):
    ...
    for record in recorded_data_api.list_records():
...
def _trimmed_high(values: Sequence[float]) -> float:
    ...
    trim_fraction = _fraction(getattr(project_config, "HTCONDOR_RESOURCE_TRIM_TOP_FRACTION", 0.05))
