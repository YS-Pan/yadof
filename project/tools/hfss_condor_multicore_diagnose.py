...
def quote_submit_atom(value: str) -> str:
...
def resolve_python_exe(explicit: str | None) -> str:
    raw = explicit or str(getattr(project_config, "HTCONDOR_PYTHON_EXE", "python"))
    if raw.strip().lower() in {"python", "python.exe"}:
        found = shutil.which(raw)
        if found:
            return windows_short_path(str(Path(found).resolve()))
    return windows_short_path(raw)


def windows_short_path(path_text: str) -> str:
    if os.name != "nt" or " " not in str(path_text):
        return str(path_text)
    try:
        import ctypes

        buffer = ctypes.create_unicode_buffer(2048)
        result = ctypes.windll.kernel32.GetShortPathNameW(str(path_text), buffer, len(buffer))
        if result:
...
def patch_runtime_config(
    *,
    ...
    values: dict[str, Any] = {
        ...
        "HTCONDOR_PYTHON_EXE": python_exe,
    ...
    *,
    ...
    submit_text = "\n".join(
        [
            ...
            "universe = vanilla",
            f"executable = {quote_submit_atom(python_exe)}",
            "initialdir = .",
            "getenv = False",
            ...
            "transfer_input_files = ._appdata,._home,._localappdata,._tmp,collect_windows_events.ps1,workflow.py",
...
def write_markdown_summary(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Python executable: `{payload.get('python_exe')}`",
        "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    python_exe = resolve_python_exe(args.python_exe or None)
