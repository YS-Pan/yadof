...
def test_real_local_pipeline_records_rawdata_without_cost_files(tmp_path, monkeypatch):
    ...
    costs = evaluate_population(((0.25, 0.5, 0.75),), jobs_dir=jobs_dir, timeout_sec=30)
    ...
    assert records[0]["raw_variables"] == pytest.approx([-1.0, 0.0, 0.75])

    history = recorded_api.get_optimization_history()
    assert len(history) == 1
    _job_name, normalized_variables, history_costs = history[0]
    assert normalized_variables == pytest.approx((0.25, 0.5, 0.75))
