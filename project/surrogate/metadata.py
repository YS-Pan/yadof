...
def record_surrogate_metadata(metadata: Mapping[str, object]) -> dict[str, object] | None:
    try:
        recorded_api = importlib.import_module("project.recorded_data.api")
    except Exception:
        return None
    record_fn = getattr(recorded_api, "record_surrogate_metadata", None)
    if callable(record_fn):
        return record_fn(dict(metadata))
    fallback = getattr(recorded_api, "record_optimization_metadata", None)
    if callable(fallback):
        return fallback(dict(metadata))
    return None
