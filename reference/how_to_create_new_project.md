...
- `project/` 保存实际代码。
- `dev_doc/` 保存当前项目的设计文档、blueprints、术语表、未来任务、变更记录等。
...
- `blueprints/` 先读取所有文件名，再按需读取相关文件全文。
...
`blueprints/` 是给 AI agent 使用的生成式模块说明。

中心思想：

> blueprint 的目标是让 AI 能够根据这个文件从零开始生成一个同样功能的模块，而不是描述当前文件里每一行代码。
...
推荐先写模块级 blueprint，不要过早给每个源文件都写一个 blueprint。文件级 blueprint 只有在某个文件有复杂、稳定、容易误解的合同时才需要。

AI 收集信息时，应先列出 `blueprints/` 所有文件名，然后根据任务选择相关文件全文读取。
...
- 它不是复制计划。
- 它是索引和血缘说明。
- 它帮助 AI 快速找到最相关的旧代码、旧 blueprint/prompt 文档、论文或调试记录。
...
如果某个 obsolete 文档重新变成当前设计依据，应把相关内容迁移到 `spec 20260502.md`、`architecture/`、`blueprints/` 或 `reference_map.md`，而不是直接把 obsolete 当作当前文档使用。
...
6. 创建 `dev_doc/`，先写 `README.md`、`spec 20260502.md`、`terminology.md`、`architecture/00_architecture_index.md`、`blueprints/00_project.md`，并建立空的 `toDo/`、`change_records/`、`obsolete/` 文件夹。
...
- 不要把 blueprint 写成当前代码摘要，要写成可重建模块功能的生成式说明。
